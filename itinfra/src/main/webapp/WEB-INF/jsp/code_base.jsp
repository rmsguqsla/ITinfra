...
<body>
  ...
  <!-- 등록하기 -->
  <form class="base_insert" action="insertBase.do" method="post" onsubmit="return CheckInsertForm()">
        ...
        <div class="forms_table_non">
          <table>
            <tr>
              <th class="th">그룹코드</th>
              <td>
              	<select class="input" name="Group_Code">
              		<c:forEach var="result" items="${groupInfoList}" varStatus="status">
                  		<option value="${result.bcode}">${result.bcode}</option>
                  	</c:forEach>
                </select>
              </td>
            </tr>
            <tr>
              <th class="th">기본코드</th>
              <td>
                <input type="text" class="input" placeholder="입력하기" value="" name="BCode" id="insert_B">
              </td>
            </tr>
            <tr>
  ...
  <form action="updateBase.do" method="post" onsubmit="return CheckUpdateForm()">
        ...
        <div class="forms_table_non">
          <table>
            <tr>
              <th class="th">그룹코드</th>
              <td>
                <input type="text" class="input" placeholder="입력하기" value="" name="Group_Code" id="re_Group_Code" readonly>
              </td>
            </tr>
            <tr>
              <th class="th">기본코드</th>
  ...
  <div class="main">
    ...
    <div class="main_body">
      ...
      <div class="contants_body">
        <form style="display: inline;" action="baseCode.do" method="post">
        <div class="gs_b_top">
          <ul>
            <li>
              <select class="input" name="searchOption">
                ...
                <c:forEach var="result" items="${searchgrouplist}" varStatus="status">
                	<option value="${result.gtitle}" <c:out value="${searchOption == result.gtitle?'selected':''}"/> >${result.gtitle}</option>
                </c:forEach>
              </select>
            </li>
            <li>
            ...
            <div class="containers">
              <div class="tabels cols2">
                <table id="examples" class="display">
                  ...
                  <tbody>
                  <c:forEach var="result" items="${baseList}" varStatus="status">
                  		<tr>
                  			<td>${result.gcode}</td>
                  			<td>${result.gtitle}</td>
                  			<td>${result.bcode}</td>
    ...
    $(document).ready(function () {
      ...
      $('#examples tr').click(function() {
    		tr = $(this);
      	  	var td = tr.children();
      	  	$('.recode').click(function () {    	  		     	  		
	      	  	$('#re_Group_Code').val(td.eq(0).text());
	      		$('#re_BCode').val(td.eq(2).text());
	      	  	$('#re_Title').val(td.eq(3).text());
	      		$('#re_Code_Order').val(td.eq(4).text());
	      		$('#re_Ext_Text1').val(td.eq(5).text());
	      		$('#re_Ext_Text2').val(td.eq(6).text());
	      		$('#re_Ext_Text3').val(td.eq(7).text());
	      		$('#re_Ext_Number').val(td.eq(8).text());
	      		$('#re_Note').val(td.eq(9).text());
	      		$('#re_Use_YN').val(td.eq(10).text());
      	  });
    	});
    });
